"""Load and validate the MIDI bridge YAML configuration."""
...
from .midimap import ConfigError, build_midi_map


class MidiConfig:
    def __init__(self, raw: dict):
        # None = the runner lists ports and lets the user pick / uses the first.
...
class BridgeConfig:
    def __init__(self, raw: dict):
        ...
        self.midi = MidiConfig(raw.get("midi", {}) or {})
        self.controller = ControllerConfig(raw.get("controller", {}) or {})
        self.midi_map = build_midi_map(raw.get("bindings", []) or [])
